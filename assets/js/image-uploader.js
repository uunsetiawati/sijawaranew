    ...
    $.fn.imageUploader = function (options) {
        ...
        let createImg = function (src, id, preloaded) {
            ...
            let $container = $('<div>', {class: 'uploaded-image'}),
                ...
                $i = $('<i>', {class: 'fa-solid fa-circle-xmark'}).appendTo($button);
            ...
            if (preloaded) {
                ...
                let $preloaded = $('<input>', {
                    ...
                    value: plugin.settings.preloaded[id].src
                }).appendTo($container)
        ...
        let validateExtension = function (file) {
            ...
            if (plugin.settings.extensions.indexOf(file.name.replace(new RegExp('^.*\\.'), '.')) < 0) {
                Toast.fire({
                    icon: 'error',
                    title: `<div class="text-wrap" style="width: 13.5rem;">The file "${file.name}" does not match with the accepted file extensions: "${plugin.settings.extensions.join('", "')}"</div>`
                })
        ...
        let validateMIME = function (file) {
            ...
            if (plugin.settings.mimes.indexOf(file.type) < 0) {
                Toast.fire({
                    icon: 'error',
                    title: `<div class="text-wrap" style="width: 13.5rem;">The file "${file.name}" does not match with the accepted mime types: "${plugin.settings.mimes.join('", "')}"</div>`
                })
        ...
        let validateMaxSize = function (file) {
            ...
            if (file.size > plugin.settings.maxSize) {
                Toast.fire({
                    icon: 'error',
                    title: `<div class="text-wrap" style="width: 13.5rem;">The file "${file.name}" exceeds the maximum size of ${plugin.settings.maxSize / 1024 / 1024}Mb </div>`
                })
        ...
        let validateMaxFiles = function (index, file) {
            ...
            if ((index + dataTransfer.items.length + plugin.settings.preloaded.length) >= plugin.settings.maxFiles) {
                Toast.fire({
                    icon: 'error',
                    title: `<div class="text-wrap" style="width: 13.5rem;">The file "${file.name}" could not be added because the limit of ${plugin.settings.maxFiles} files was reached </div>`
                })
